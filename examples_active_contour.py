...
def example_nodule2():
    img = imread("./images/21.png")[..., 0] / 255.0
    lab = imread("./images/21_lab.png")[...,:3]
    color_code = np.array([255,0,0]).reshape([1,1,-1])
    # print(color_code.shape,lab.shape)
    equal = lab == color_code
    init_ls = np.all(equal,axis=-1).astype(np.int8)

    gimg = ms.inverse_gaussian_gradient(img, alpha=10, sigma=10.48)
    print(gimg.shape)
    # Initialization of the level-set.
    # init_ls = ms.circle_level_set(img.shape, (100, 126), 20)
    # print(init_ls.max(), init_ls.min(), init_ls.dtype, img.shape)
    plt.imshow(gimg)
    plt.show()
    ...
    # MorphGAC.
    ms.morphological_geodesic_active_contour(gimg, iterations=50,
